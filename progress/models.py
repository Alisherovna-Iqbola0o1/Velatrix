...
class LessonAttempt(models.Model):
    ...
    result = models.CharField(
        max_length=20, choices=RESULT_CHOICES, default="in_progress"
    )
...
class QuestionAttempt(models.Model):
    ...
    lesson_attempt = models.ForeignKey(
        LessonAttempt, on_delete=models.CASCADE, related_name="question_attempts"
    )
...
class LessonChatMessage(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    lesson_attempt = models.ForeignKey(
        LessonAttempt, on_delete=models.CASCADE, related_name="chat_messages"
    )
...
class ExtremalTestAttempt(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.user} - {self.score}/50 ({'passed' if self.passed else 'failed'})"
        )
