...
from .models import LessonProgress, LessonAttempt, QuestionAttempt, ExtremalTestAttempt
...
def start_lesson_attempt(user, lesson):
    ...
    return LessonAttempt.objects.create(
        user=user, lesson=lesson, attempt_number=attempt_count + 1, result='in_progress',
    )
...
def submit_answer(user, attempt, question, given_answer):
    ...
    QuestionAttempt.objects.create(
        lesson_attempt=attempt, question=question,
    ...
    if correct_so_far >= total_questions:
        ...
        award_xp(user, attempt.lesson.xp_reward, f"Dars tugallandi: {attempt.lesson}", related_lesson=attempt.lesson)
...
def _check_answer(question, given_answer):
    if question.question_type in ('open_answer', 'code_challenge', 'true_false'):
        return (question.correct_answer or '').strip().lower() == (given_answer or '').strip().lower()
    if question.question_type == 'multiple_choice':
        correct_choice = question.choices.filter(is_correct=True).first()
        return correct_choice is not None and str(correct_choice.id) == str(given_answer)
    return False
...
def submit_extremal_test(user, token, answers):
    try:
        data = signing.loads(token, salt=EXTREMAL_TOKEN_SALT, max_age=EXTREMAL_TOKEN_MAX_AGE)
    ...
    wrong_count = sum(
        1 for item in valid_answers
        if not questions_map.get(item['question_id']) or not _check_answer(questions_map[item['question_id']], item['answer'])
    )
    passed = wrong_count <= 3

    ExtremalTestAttempt.objects.create(
        user=user, score=len(valid_answers) - wrong_count,
        wrong_count=wrong_count, passed=passed,
    )
